...
At [event name], we spoke about [topic]. These included:

• [Pain point 1]
• [Pain point 2]
• [Pain point 3]

[Single direct question about their current state]
...
- Reference the event by name AND a specific detail (venue, host, what you were eating/drinking, a comment they made)
- NEVER open with a generic topic label ("identity risks", "compliance challenges", "pipeline reliability"). Instead, reference a specific moment, question they asked, or detail from the conversation that only they would remember.
- List exactly 3 pain points as bullets
- End with ONE question (not a pitch)
...
- **Links:** One link per email max. Embed in text, not bare URL.
